...
db = SQLAlchemy(app)
socketio = SocketIO(app, cors_allowed_origins=SITE_CONFIG["YOUR_DOMAIN"] if is_production else "*")
mail = Mail(app)
...
def class_member_required(f):
    ...
    def decorated_function(class_id, *args, **kwargs):
        ...
        if current_user != target_class.teacher and current_user not in target_class.students:
...
<body class="text-gray-200 antialiased">
    ...
    <template id="template-admin-settings-view">
        <div class="glassmorphism p-4 rounded-lg">
            ...
            <form id="admin-settings-form" class="space-y-4">
                <div>
                    <label for="site-wide-theme-select" class="block text-sm font-medium text-gray-300 mb-1">Site-Wide Theme</label>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        let appState = { currentUser: null, isLoginView: true, selectedRole: 'student', currentTab: 'my-classes', selectedClass: null, siteSettings: {} };
        ...
        function applyTheme(userPreference) { 
            ...
            const t = themes[themeToApply] || themes.golden; 
        ...
        async function apiCall(endpoint, options = {}) { try { const csrfToken = document.querySelector('meta[name="csrf-token"]').content; if (!options.headers) options.headers = {}; options.headers['X-CSRFToken'] = csrfToken; if (options.body && typeof options.body === 'object' && !(options.body instanceof FormData)) { options.headers['Content-Type'] = 'application/json'; options.body = JSON.stringify(options.body); } const response = await fetch(`/api${endpoint}`, { credentials: 'include', ...options }); const data = await response.json(); if (!response.ok) { if (response.status === 401 && endpoint !== '/status') handleLogout(false); throw new Error(data.error || `Request failed`); } return { success: true, ...data }; } catch (error) { showToast(error.message, 'error'); return { success: false, error: error.message }; } }
        ...
        function viewClass(classId) {
            ...
            selectedClassView.innerHTML = `<h3 class="text-2xl">Class Details</h3><p>Content for class ID ${classId} will appear here.</p>`;
        ...
        async function main() {
            ...
            setTimeout(async () => {
                ...
                if (result.success) {
                    appState.siteSettings = result.settings;
                    if (result.settings.maintenance_mode === "true" && (!result.user || result.user.role !== 'admin')) {
...
def render_spa(path=None):
    return render_template_string(HTML_CONTENT, g=g)
...
def update_admin_settings():
    ...
    for key, value in data.items():
        ...
        if setting:
            ...
            setting = SiteSettings(key=bleach.clean(key), value=bleach.clean(value))
