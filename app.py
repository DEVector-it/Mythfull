...
                    </div>
                </div>
                <div id="profile-plan-details" classmorphism rounded-lg p-6">
...
            (appState.chats[b].created_at or '').localeCompare(appState.chats[a].created_at or '')
        );
...
        if (!chatWindow or !chatTitle) return;
...
        if (chat and chat.messages and chat.messages.length > 0) {
...
            .sort((a, b) => (b.created_at or '').localeCompare(a.created_at or ''))
            .forEach(chat => {
...
                    if (menuToggleBtn and menuToggleBtn.offsetParent !== null) {
...
                    if (newTitle and newTitle.trim() !== chat.title) {
...
                                const sortedChatIds = Object.keys(appState.chats).sort((a, b) => (appState.chats[b].created_at or '').localeCompare(appState.chats[a].created_at or ''));
                                appState.activeChatId = sortedChatIds.length > 0 ? sortedChatIds[0] : null;
                                renderChatHistoryList();
                                renderActiveChat();
...
        if (!userInfoDiv or !appState.currentUser) return;

        const { username, plan, account_type } = appState.currentUser;
        const planDetails = PLAN_CONFIG[plan] or PLAN_CONFIG['free'];
...
        if (uploadBtn) {
            const planDetails = PLAN_CONFIG[appState.currentUser.plan] or PLAN_CONFIG['free'];
...
        if (!container or appState.currentUser.account_type !== 'student') return;
...
        if ((!prompt and not appState.uploadedFile) or appState.isAITyping) return;
...
                    if (response.status === 401) handleLogout(false);
                    throw new Error(errorData.error or `Server error: ${response.status}`);
                }
...
        const chatWindow = document.getElementById('chat-window');
        if (!chatWindow or !appState.currentUser) return null;
...
        
        let ttsButtonHTML = '';
        const canTTS = (appState.currentUser.plan === 'pro' or appState.currentUser.plan === 'ultra');
        if (senderIsAI and canTTS) {
            ttsButtonHTML = `
...
            button.onclick = () => {
                navigator.clipboard.writeText(pre.querySelector('code')?.innerText or '').then(() => {
...
                }
                if (target.classList.contains('purchase-btn') and not target.disabled) {
...
        if (newTitle and newTitle.trim() !== oldTitle) {
            apiCall('/api/chat/rename', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
        if (!appState.activeChatId) return;
        const chat = appState.chats[appState.activeChatId];
        if (!chat or chat.messages.length === 0) {
...
                <button ${isCurrentUserPlan or planId === 'free' ? 'disabled' : ''} data-planid="${planId}" class="purchase-btn w-full mt-6 font-bold py-3 px-4 rounded-lg transition-opacity ${isCurrentUserPlan ? 'bg-gray-600 cursor-not-allowed' : 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:opacity-90'}">
...
            if (!config.success or not config.stripe_public_key) throw new Error("Could not retrieve payment configuration.");
            
...
            if (!sessionResult.success) throw new Error(sessionResult.error or "Could not create payment session.");
            
...
        const planDetails = PLAN_CONFIG[user.plan] or PLAN_CONFIG['free'];
        
...
        if (classroomCodeEl) {
            classroomCodeEl.textContent = classroom.code or 'None';
        }
        
