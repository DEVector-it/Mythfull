...
from markupsafe import escape # Use Jinja2's built-in escape to ensure all HTML is safe
...
if os.environ.get('FLASK_ENV') == 'production':
    required_secrets = ['SECRET_KEY', 'DATABASE_URL', 'SECURITY_PASSWORD_SALT']
    for key in required_secrets:
        if not app.config.get(key):
            logging.critical(f"Missing a required production secret: {key}")
            exit(1)
...
csp = {
    ...
    'script-src': [
        ...
        'https://www.googletagmanager.com' # Required for Google AdSense
    ],
    'style-src': [
        '\'self\'',
        '\'unsafe-inline\'',
...
Talisman(app, content_security_policy=csp, session_cookie_secure=True)
...
class User(UserMixin, db.Model):
    ...
    password_hash = db.Column(db.String(256), nullable=False) # Should not be nullable
...
class Question(db.Model):
    ...
    choices = db.Column(db.JSON, nullable=False) # e.g., {'options': ['A', 'B', 'C'], 'correct': 'B'}
...
# ==============================================================================
# The HTML and JS are now handled by a separate function to ensure all dynamic
# content is properly escaped before rendering.
def get_safe_html_content():
    # Use f-string with escaping for any Python variables
    return f"""
<!DOCTYPE html>
...
<head>
    ...
    <meta name="csrf-token" content="{{{ csrf.generate_csrf() }}}">
</head>
<body class="text-gray-200 antialiased">
    ...
    <template id="template-full-screen-loader">...</template>
    <template id="template-maintenance-page">...</template>
    <template id="template-role-choice">...</template>
    <template id="template-main-dashboard">...</template>
    <template id="template-auth-form">...</template>
    ...
    <template id="template-class-whiteboard-view">...</template>
    ...
    <template id="template-privacy-policy">...</template>
    <template id="template-plans">...</template>
    <template id="template-contact-form">...</template>
    ...
    document.addEventListener('DOMContentLoaded', () => {{
        ...
        function switchClassView(view) {{ document.querySelectorAll('.class-view-tab').forEach(t => t.classList.toggle('active-tab', t.dataset.tab === view)); const container = document.getElementById('class-view-content'); if (view === 'chat') {{ renderSubTemplate(container, 'template-class-chat-view', async () => {{ document.getElementById('chat-form').addEventListener('submit', handleSendChat); const result = await apiCall(`/class_messages/${{escape(appState.selectedClass.id)}}`); if (result.success) {{ const messagesDiv = document.getElementById('chat-messages'); messagesDiv.innerHTML = ''; result.messages.forEach(m => appendChatMessage(m)); getSmartReplies(); }} }}); }} else if (view === 'assignments') {{ renderSubTemplate(container, 'template-class-assignments-view', async () => {{ const list = document.getElementById('assignments-list'); const actionContainer = document.getElementById('assignment-action-container'); if(appState.currentUser.role === 'teacher') {{ actionContainer.innerHTML = `<button id="create-assignment-btn" class="brand-gradient-bg shiny-button text-white font-bold py-2 px-4 rounded-lg">New Assignment</button>`; document.getElementById('create-assignment-btn').addEventListener('click', handleCreateAssignment); }} const result = await apiCall(`/classes/${{escape(appState.selectedClass.id)}}/assignments`); if(result.success) {{ if(result.assignments.length === 0) list.innerHTML = `<p class="text-gray-400">No assignments posted yet.</p>`; else list.innerHTML = result.assignments.map(a => `<div class="p-4 bg-gray-800/50 rounded-lg cursor-pointer" data-id="${{escape(a.id)}}"><div class="flex justify-between items-center"><h6 class="font-bold text-white">${{escape(a.title)}}</h6><span class="text-sm text-gray-400">Due: ${{new Date(a.due_date).toLocaleDateString()}}</span></div>${{appState.currentUser.role === 'student' ? (a.student_submission ? `<span class="text-xs text-green-400">Submitted</span>` : `<span class="text-xs text-yellow-400">Not Submitted</span>`) : `<span class="text-xs text-cyan-400">${{escape(String(a.submission_count))}} Submissions</span>`}}</div>`).join(''); list.querySelectorAll('div[data-id]').forEach(el => el.addEventListener('click', e => viewAssignmentDetails(e.currentTarget.dataset.id))); }} }}); }} else if (view === 'quizzes') {{ renderSubTemplate(container, 'template-class-quizzes-view', async () => {{ const list = document.getElementById('quizzes-list'); const actionContainer = document.getElementById('quiz-action-container'); if(appState.currentUser.role === 'teacher') {{ actionContainer.innerHTML = `<button id="create-quiz-btn" class="brand-gradient-bg shiny-button text-white font-bold py-2 px-4 rounded-lg">New Quiz</button>`; document.getElementById('create-quiz-btn').addEventListener('click', handleCreateQuiz); }} const result = await apiCall(`/classes/${{escape(appState.selectedClass.id)}}/quizzes`); if(result.success) {{ if(result.quizzes.length === 0) list.innerHTML = `<p class="text-gray-400">No quizzes posted yet.</p>`; else list.innerHTML = result.quizzes.map(q => `<div class="p-4 bg-gray-800/50 rounded-lg cursor-pointer" data-id="${{escape(q.id)}}"><div class="flex justify-between items-center"><h6 class="font-bold text-white">${{escape(q.title)}}</h6><span class="text-sm text-gray-400">${{escape(String(q.time_limit))}} mins</span></div>${{appState.currentUser.role === 'student' ? (q.student_attempt ? `<span class="text-xs text-green-400">Attempted - Score: ${{escape(q.student_attempt.score.toFixed(2))}}%</span>` : `<span class="text-xs text-yellow-400">Not Attempted</span>`) : ``}}</div>`).join(''); list.querySelectorAll('div[data-id]').forEach(el => el.addEventListener('click', e => viewQuizDetails(e.currentTarget.dataset.id))); }} }}); }} else if (view === 'students') {{ renderSubTemplate(container, 'template-class-students-view', () => {{ document.getElementById('class-students-list').innerHTML = appState.selectedClass.students.map(s => `<li class="flex items-center gap-3 p-2 bg-gray-800/50 rounded-md"><img src="${{escape(s.profile.avatar || `https://i.pravatar.cc/40?u=${{s.id}}`)}}" class="w-8 h-8 rounded-full"><span>${{escape(s.username)}}</span></li>`).join(''); }}); }} }}
        async function handleSendChat(e) {{ e.preventDefault(); const input = document.getElementById('chat-input'); const button = document.getElementById('send-chat-btn'); const message = input.value.trim(); if (!message) return; input.value = ''; document.getElementById('smart-reply-container').innerHTML = ''; const result = await apiCall('/chat/send', {{ method: 'POST', body: {{ prompt: message, class_id: appState.selectedClass.id }} }}); }}
        function appendChatMessage(message) {{ const messagesDiv = document.getElementById('chat-messages'); if (!messagesDiv) return; const isCurrentUser = message.sender_id === appState.currentUser.id; const isAI = message.sender_id === null; const msgWrapper = document.createElement('div'); msgWrapper.className = `flex items-start gap-3 ${{isCurrentUser ? 'user-message justify-end' : 'ai-message justify-start'}}`; const avatar = `<img src="${{escape(message.sender_avatar || (isAI ? aiAvatarSvg : `https://i.pravatar.cc/40?u=${{message.sender_id}}`))}}" class="w-8 h-8 rounded-full">`; const bubble = `<div class="flex flex-col"><span class="text-xs text-gray-400 ${{isCurrentUser ? 'text-right' : 'text-left'}}">${{escape(message.sender_name || (isAI ? 'AI Assistant' : 'User'))}}</span><div class="chat-bubble p-3 rounded-lg border mt-1 max-w-md text-white">${{escape(message.content)}}</div><span class="text-xs text-gray-500 mt-1 ${{isCurrentUser ? 'text-right' : 'text-left'}}">${{new Date(message.timestamp).toLocaleTimeString()}}</span></div>`; msgWrapper.innerHTML = isCurrentUser ? bubble + avatar : avatar + bubble; messagesDiv.appendChild(msgWrapper); messagesDiv.scrollTop = messagesDiv.scrollHeight; }}
        async function getSmartReplies() {{ const result = await apiCall('/smart_replies', {{ method: 'POST', body: {{ class_id: appState.selectedClass.id }} }}); if (result.success && result.replies) {{ const container = document.getElementById('smart-reply-container'); container.innerHTML = result.replies.map(reply => `<button class="smart-reply-btn bg-gray-700 hover:bg-gray-600 text-xs py-1 px-3 rounded-full">${{escape(reply)}}</button>`).join(''); container.querySelectorAll('.smart-reply-btn').forEach(btn => btn.addEventListener('click', () => {{ document.getElementById('chat-input').value = btn.textContent; document.getElementById('chat-form').requestSubmit(); }})); }} }}
        async function main() {{ const maintenanceStatus = await apiCall('/maintenance_status'); if (maintenanceStatus.success && maintenanceStatus.enabled) {{ setupMaintenancePage(); return; }} const path = window.location.pathname; if (path.startsWith('/admin-signup/')) {{ const token = path.split('/')[2]; setupAuthPage(token); }} else {{ const status = await apiCall('/status'); if (status.success && status.user) {{ appState.currentUser = status.user; applyTheme(status.user.theme_preference || 'golden'); setupDashboard(status.user, status.settings); }} else {{ setupRoleChoicePage(); }} }} }}
        ...
        async function handleManageBilling() {{ const result = await apiCall('/create-portal-session', {{ method: 'POST', body: {{}} }}); if (result.success && result.url) {{ window.location.href = result.url; }} }}
        async function handleAdminUserAction(action, userId) {{ if (action === 'delete') {{ if (!confirm('Are you sure?')) return; const result = await apiCall(`/admin/users/${{escape(userId)}}`, {{ method: 'DELETE' }}); if (result.success) {{ showToast('User deleted.', 'success'); switchAdminView('users'); }} }} else if (action === 'edit') {{ showToast('Edit user is not yet implemented.', 'info'); }} }}
        async function handleAdminDeleteClass(classId) {{ if (!confirm('Are you sure?')) return; const result = await apiCall(`/admin/classes/${{escape(classId)}}`, {{ method: 'DELETE' }}); if (result.success) {{ showToast('Class deleted.', 'success'); switchAdminView('classes'); }} }}
        ...
        async function handleToggleMaintenance() {{ const result = await apiCall('/admin/toggle_maintenance', {{ method: 'POST', body: {{}} }}); if (result.success) showToast(`Maintenance mode ${{result.enabled ? 'enabled' : 'disabled'}}.`, 'success'); }}
        async function handleGenerateAdminLink() {{ const result = await apiCall('/admin/generate-signup-link', {{ method: 'POST', body: {{}} }}); if (result.success && result.url) {{ const display = document.getElementById('admin-link-display'); const input = display.querySelector('input'); input.value = result.url; display.classList.remove('hidden'); input.select(); document.execCommand('copy'); showToast('Signup link copied to clipboard!', 'success'); }} }}
        ...
        async function handleDeleteMusic(musicId) {{ if (!confirm('Are you sure?')) return; const result = await apiCall(`/admin/music/${{escape(musicId)}}`, {{ method: 'DELETE' }}); if (result.success) {{ showToast('Music deleted.', 'success'); switchAdminView('music'); }} }}
        ...
        async function handleCreateQuiz() {{ const topic = prompt("Enter a topic for the AI to generate a quiz about:"); if (topic) {{ const result = await apiCall('/generate_quiz', {{ method: 'POST', body: {{ topic, class_id: appState.selectedClass.id }} }}); if (result.success) {{ showToast('AI-generated quiz created!', 'success'); switchClassView('quizzes'); }} }} }}
        async function viewQuizDetails(quizId) {{ showToast('View quiz details is not yet implemented.', 'info'); }}
        function setupStudyGuideTab(container) {{ renderSubTemplate(container, 'template-study-guide-tab', () => {{ document.getElementById('generate-guide-btn').addEventListener('click', handleGenerateStudyGuide); }}); }}
        async function handleGenerateStudyGuide() {{ const topic = document.getElementById('study-guide-topic').value; if (!topic) {{ showToast('Please enter a topic.', 'error'); return; }} const result = await apiCall('/generate_study_guide', {{ method: 'POST', body: {{ topic }} }}); if (result.success) {{ document.getElementById('study-guide-content').innerHTML = result.guide; }} }}
...
def reset_password_page(token):
    ...
    # Render the password reset form using the same safe templating method
...
def signup():
    ...
    admin_signup_token = data.get('admin_signup_token')
    if admin_signup_token:
        token_record = AdminSignupToken.query.filter_by(token=admin_signup_token, is_used=False).first()
        if not token_record:
            return jsonify(error='Invalid or expired admin signup link.'), 403
        if account_type != 'admin':
            return jsonify(error='Admin signup link can only be used for admin accounts.'), 400
        
        token_record.is_used = True
    elif account_type == 'teacher' and data.get('secret_key') != SITE_CONFIG.get('SECRET_TEACHER_KEY'):
        return jsonify(error='Invalid secret key for teacher account.'), 403
    elif account_type == 'admin' and not admin_signup_token:
        return jsonify(error='Admin accounts can only be created via a special signup link.'), 403
...
def login():
    ...
    if user.role == 'admin' and data.get('admin_secret_key') != SITE_CONFIG.get('ADMIN_SECRET_KEY'):
...
def status():
    ...
    return jsonify(success=False, user=None, settings=settings)

@app.route('/api/maintenance_status')
...
def request_password_reset():
    ...
    if user:
        try:
            ...
            logging.error(f"Failed to send password reset email for {email}: {str(e)}")
...
def reset_password():
    ...
    if not token or not new_password or len(new_password) < 8:
        return jsonify(error='Missing or invalid token/new password.'), 400
    try:
        email = password_reset_serializer.loads(token, salt=app.config['SECURITY_PASSWORD_SALT'], max_age=3600)
...
def user_has_class_access(class_id, user):
    if not class_id or not user or not user.is_authenticated:
        return False
...
def create_class():
    ...
    if len(name) > 100:
        return jsonify(error='Class name is too long.'), 400
    ...
    try:
        ...
        new_class = Class(name=name, code=code, teacher_id=current_user.id)
...
def send_chat_and_get_ai_response():
    ...
    try:
        ...
        ai_response_text = f"AI response to: '{prompt}'"
...
def generate_quiz():
    ...
    class_id = data.get('class_id')
    if not topic or not class_id:
        return jsonify(error="Topic and class ID are required."), 400
    ...
    try:
        ...
        ai_generated_questions = [
            {
                "text": f"What is the capital of {topic}?",
                "choices": {"options": ["Option A", "Option B", "Correct Answer"], "correct": "Correct Answer"}
            },
            {
                "text": f"What is a key feature of {topic}?",
                "choices": {"options": ["Feature 1", "Correct Feature", "Feature 3"], "correct": "Correct Feature"}
            }
        ]

        new_quiz = Quiz(
            title=f"AI-Generated Quiz: {topic}",
            description=f"A quiz about {topic} generated by AI.",
            time_limit=15,
            class_id=class_id
        )
        ...
        for q_data in ai_generated_questions:
            question = Question(
                quiz_id=new_quiz.id,
                text=q_data["text"],
        ...
        db.session.commit()
        return jsonify(success=True, message="AI quiz generated successfully.")
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error generating AI quiz: {e}")
        return jsonify(error="Failed to generate AI quiz."), 500
...
def grade_assignment():
    ...
    cls = Class.query.get(submission.assignment.class_id)
    if not cls or (current_user.id != cls.teacher_id and current_user.role != 'admin'):
        return jsonify(error="Permission denied. You must be the teacher or an admin to grade."), 403

    try:
        # MOCK AI GRADING
        content = submission.content
        mock_grade = 85.5
        mock_feedback = "This is a good start. Consider elaborating on your main points with more specific examples. Your conclusion could also be stronger by summarizing the key arguments."
        
        submission.grade = mock_grade
        submission.feedback = mock_feedback
        db.session.commit()

        notification = Notification(user_id=submission.student_id, content=f"Your assignment '{submission.assignment.title}' has been graded.")
        db.session.add(notification)
        db.session.commit()
        socketio.emit('new_notification', {'content': notification.content}, room=f'user_{submission.student_id}')
        
        return jsonify(success=True, grade=mock_grade, feedback=mock_feedback)
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error during AI grading: {e}")
        return jsonify(error="Failed to grade assignment with AI."), 500


@app.route('/api/smart_replies', methods=['POST'])
@login_required
@csrf.exempt
def get_smart_replies():
    ...
    mock_replies = ["Thanks!", "I understand.", "Can you explain that further?"]
    return jsonify(success=True, replies=mock_replies)
...
def create_team():
    ...
    try:
        ...
        while Team.query.filter_by(code=code).first():
            code = secrets.token_urlsafe(4).upper()
        new_team = Team(name=name, code=code, owner_id=current_user.id)
...
def update_profile():
    ...
    try:
        ...
        avatar = data.get('avatar', profile.avatar)
        theme_preference = data.get('theme_preference', current_user.theme_preference)
...
def stripe_webhooks():
    ...
    webhook_secret = SITE_CONFIG.get('STRIPE_WEBHOOK_SECRET')
    event = None
    ...
    if event['type'] == 'checkout.session.completed':
      ...
      if user_id:
        ...
        if user:
            ...
            db.session.commit()
            logging.info(f"Subscription deactivated for user {user.id}")
...
@app.route('/api/admin/generate-signup-link', methods=['POST'])
@admin_required
@csrf.exempt
def generate_admin_signup_link():
    try:
        token_str = secrets.token_urlsafe(32)
        new_token = AdminSignupToken(token=token_str, created_at=datetime.utcnow(), is_used=False)
        db.session.add(new_token)
        db.session.commit()
        signup_url = url_for('admin_signup_page', token=token_str, _external=True)
        return jsonify(success=True, url=signup_url)
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error generating admin signup link: {e}")
        return jsonify(error="Could not generate signup link."), 500
...
def admin_manage_music():
    if request.method == 'POST':
        ...
        try:
            music = BackgroundMusic(name=name, url=url, uploaded_by=current_user.id)
...
with app.app_context():
    ...
    admin_username = 'bigballz'
    admin_password = os.environ.get('ADMIN_PASSWORD_INIT') or 'change_me_in_prod'
    if not User.query.filter_by(username=admin_username).first():
        try:
            ...
            db.session.commit()
            logging.info("Default admin user created. Please change the password immediately in production.")
        except IntegrityError:
            db.session.rollback()
